...
namespace GameServer
{
    ...
    WORD CGameServer::GetMyServerBodyID()
    {
        return *(WORD*)MEMUTIL_ADD_PTR(*reinterpret_cast<uintptr_t*>(0x010F6E54), 0);
    }
    ...
    void CGameServer::SendMsgToServerBody(E_MODULE::Type Type, CMsg* pMsg)
    {
        if (!pMsg) return;
        ...
        for (std::vector<WORD>::const_iterator it = serverBodyList.begin(); it != serverBodyList.end(); ++it)
        {
            ...
            if (!reinterpret_stdcall(0x00BE0650, char, static_cast<int>(serverBodyID), pMsg))
