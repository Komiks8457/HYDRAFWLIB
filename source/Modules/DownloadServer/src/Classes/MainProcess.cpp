...
namespace DownloadServer
{
    ...
    {
        MEMUTIL_VFTABLE_HOOK(0x0151AAF4, 7, &CMainProcess::PrepareMsgHandlers);
        MEMUTIL_VFTABLE_HOOK(0x0151AAF4, 8, &CMainProcess::RegisterMsgHandler);
        MEMUTIL_VFTABLE_HOOK(0x0151AAF4, 9, &CMainProcess::HandleMsg);
    }

    BOOL CMainProcess::PrepareMsgHandlers()
    {
        return reinterpret_thiscall(0x01432600, BOOL, this);
    }

    BOOL CMainProcess::RegisterMsgHandler(WORD wMsgID, uintptr_t fnPtr)
    {
        return reinterpret_thiscall(0x01447DE0, BOOL, this, wMsgID, fnPtr);
    }
