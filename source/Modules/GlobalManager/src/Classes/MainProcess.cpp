...
{
    DWORD OnHandleMsg_returnAddress = 0x0173B815;
    ...
    {
        MEMUTIL_VFTABLE_HOOK(0x018053CC, 7, &CMainProcess::PrepareMsgHandlers);
        MEMUTIL_VFTABLE_HOOK(0x018053CC, 8, &CMainProcess::RegisterMsgHandler);

        // TODO: disable for a moment since vanguard is hooked to mainprocess. We'll find another way later.
        //MEMUTIL_VFTABLE_HOOK(0x018053CC, 9, &CMainProcess::HandleMsg);
        MEMUTIL_SETUP_HOOK(LongJump, 0x0173B810, OnHandleMsg);
    }
    ...
    BOOL CMainProcess::PrepareMsgHandlers()
    {
        ...
        return reinterpret_thiscall(0x0169AD90, BOOL, this);
    }

    BOOL CMainProcess::RegisterMsgHandler(WORD wMsgID, uintptr_t fnPtr)
    {
        return reinterpret_thiscall(0x0173AD60, BOOL, this, wMsgID, fnPtr);
    }

    BOOL CMainProcess::HandleMsg(CMsg *pMsg, DWORD a3, LPVOID a4, CMassiveMsg *pMassiveMsg)
    {
        if (pMsg && pMsg->GetMsgID() == MODULE_CERTIFICATION_ACK && pMsg->GetMsgSize() > 400)
    ...
    BOOL CMainProcess::CustomMsg(CMsg *pMsg, DWORD, LPVOID, CMassiveMsg*)
    {
        ...
        switch (wMsgID) {
            ...
            default: {
                ...
                reinterpret_cdecl(0x01706700, char*, buffer, pMsg);
