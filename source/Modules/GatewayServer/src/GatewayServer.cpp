...
namespace GatewayServer
{
    ...
    WORD CGatewayServer::GetMyServerBodyID()
    {
        return *(WORD*)MEMUTIL_ADD_PTR(*reinterpret_cast<uintptr_t*>(0x015D2DD8), 0);
    }
    ...
    void CGatewayServer::SendMsgToServerBody(E_MODULE::Type Type, CMsg* pMsg)
    {
        if (!pMsg) return;
        ...
        for (std::vector<WORD>::const_iterator it = serverBodyList.begin(); it != serverBodyList.end(); ++it)
        {
            ...
            if (!reinterpret_stdcall(0x0173C4F0, char, static_cast<int>(serverBodyID), pMsg))
